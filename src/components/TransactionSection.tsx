...
export default function TransactionSection({ openDemo }: Props) {
  return (
    <section
      id="platform"
      className="relative py-28 overflow-hidden bg-gradient-to-br from-[#060714] via-[#0b0e2a] to-[#120c2f]"
    >
      {/* Grid Background */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8 md:px-16 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div>
          ...
          <Button
            size="lg"
            onClick={openDemo}
            className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 shadow-lg shadow-purple-500/30 transition"
          >
            Request Demo →
          </Button>
        </div>
...
function FloatingCard({ top, left, right, delay }: any) {
  return (
    <motion.div
      ...
      animate={{ scale: [0.4, 1, 1], opacity: [0, 1, 0.85, 1] }}
      transition={{ duration: 3, repeat: Infinity, repeatDelay: 2, delay }}
    />
  );
}

/* Feature */
function Feature({ icon: Icon, text, color }: any) {
  return (
